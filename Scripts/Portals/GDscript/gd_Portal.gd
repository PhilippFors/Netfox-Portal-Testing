...
#@export var _tracked_phys_bodies = []
#
...
#func _update_portal_camera_near_clip_plane(camera):
	...
	#camera.oblique_normal = clip_plane_forward
	#camera.oblique_position = other_portal.global_position
#
...
#
		...
		#return
#
	#$CameraViewport/Camera3D.global_transform = real_to_exit_transform(cur_camera.global_transform)
...
#func real_to_exit_transform(real:Transform3D) -> Transform3D:
	...
	#var exit_scale_vector:Vector3 = other_portal.global_transform.basis.get_scale()
	...
	#var local_at_exit:Transform3D = other_portal.global_transform * scaled_at_exit
...
#func real_to_exit_position(real:Vector3) -> Vector3:
	...
	## Apply any scale the exit portal may have (and apply custom exit scale)
	#var exit_scale_vector:Vector3 = Vector3(-1, 1, 1) * other_portal.global_transform.basis.get_scale()
	#var scaled_at_exit:Vector3 = unscaled / exit_scale_vector * exit_scale
	## Convert from local space at the exit portal to global space
	#var local_at_exit:Vector3 = other_portal.global_transform * scaled_at_exit
	#return local_at_exit
#
#func real_to_exit_rotation(real:Vector3) -> Vector3:
	#var local:Vector3 = global_transform.affine_inverse() * real
	## Compensate for any scale the entrance portal may have
	#var unscaled:Vector3 = local * global_transform.basis.get_scale()
	## Flip it (the portal always flips the view 180 degrees)
...
#func real_to_exit_direction(real:Vector3) -> Vector3:
	...
	## Convert from local space at the exit portal to global space
	#var local_at_exit:Vector3 = other_portal.global_transform.basis * scaled_at_exit
	#return local_at_exit
#
...
#func _move_to_other_portal(body: PhysicsBody3D):
	...
	#traveller.travel(self, other_portal)
...
#func _get_bodies_which_passed_through_this_frame():
	...
	#for tracked_body in _tracked_phys_bodies:
		#var traveller = NodeHelper.find_child_node(tracked_body.body, PortalTraveller) as PortalTraveller
		#var pos_node   = traveller.camera if traveller.camera else tracked_body.body
		...
		#var portal_side             = offset_from_portal.dot(forward)
		#var prev_portal_side        = prev_offset_from_portal.dot(forward)
		#if portal_side < 0 && prev_portal_side > 0:
...
#func _on_body_entered(body):
	...
	## CSGShape3Ds are also static if you enable their use_collision property so disable them.
		...
		#var portalObject = NodeHelper.find_child_node(body, PortalTraveller) as PortalTraveller
		#if !portalObject:
			#print("No portal object")
			#return
		#if _check_shapecast_collision(body):
			#_add_tracked_phys_body(body)
			##portalObject.onPortalEnter.emit()
#
#func _on_body_exited(body):
	...
	#_remove_tracked_phys_body(body)
	##portalObject.onPortalExit.emit()
