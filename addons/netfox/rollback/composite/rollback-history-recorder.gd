...
func record_state(tick: int, node: String) -> void:
	...
	#if(record_state.has(":position") and not node == "Player 1"):
		#_logger.debug(str(node ,": Record of tick: ", tick, ": ", record_state.get_value(":position")))
